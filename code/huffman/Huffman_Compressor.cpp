...
void Huffman_Compressor::_free_tree()
{
    _delete_subroot(_root);
    _root = nullptr;
}

void Huffman_Compressor::_delete_subroot(_Node* subroot)
{
    if (subroot == nullptr)
        return;

    _delete_subroot(subroot->_Left);
    _delete_subroot(subroot->_Right);
    delete subroot;
}
